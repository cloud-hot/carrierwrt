...
--- Build the complete URL, including the http[s]:// part, hostname and port.
-- @param ...	Virtual path
-- @return 		Complete URL
function build_complete_url(...)
	local protocol = http.getenv('HTTPS') and "https://" or "http://"
	local host = http.getenv('HTTP_HOST') or http.getenv('SERVER_ADDR')
	local port = http.getenv('SERVER_PORT')

	if http.getenv('HTTPS') and port ~= 443 or not http.getenv('HTTPS') and port ~= 80 then
		port = ':' .. port
	else
		port = ''
	end
		...
		if not util.contains(accs, user) then
			if authen then
				...
				if not user or not util.contains(accs, user) then
					...
					local sid = sess or luci.sys.uniqueid(16):lower()
					if not sess then
						local token = luci.sys.uniqueid(16):lower()
						sauth.write(sid, util.get_bytecode({
							user=user,
							token=token,
							secret=luci.sys.uniqueid(16):lower()
						}))
...
function fon_gen_env()
	env = {}
	env.uci = require("luci.model.uci").cursor()
	env.uci_state = require("luci.model.uci").cursor_state()
	env.mounted = env.uci_state:get("mountd", "mountd", "count") or "0"
	env.online = (tonumber((env.uci_state:get("fon", "state", "online") or "0")) > 0) and true or false
	return env
end
...
local function _cbi(self, ...)
	...
	for i, res in ipairs(maps) do
		...
		if res.done_btn == true then
			done_btn = true
		end
		if res.cancelaction == true then
			cancelaction = true
		end
	end
	if not config.nofooter then
		tpl.render("cbi/footer", {flow = config, pageaction=pageaction, cancelaction=cancelaction, state = state, done_btn=done_btn, autoapply = config.autoapply})
	end
end
