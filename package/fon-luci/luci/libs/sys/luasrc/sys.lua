...
--- Get mac of a device
-- @param dev	String containing a network devie
-- @return	    mac on success and nil on fail	
function net.getmac(dev)
	local cnt = luci.util.exec("cat /sys/class/net/"..dev.."/address 2>/dev/null")
...
--- Get ifconfig output for all devices.
-- @return	Table of tables containing the ifconfig output for each net device
function net.getifconfig()
	local cnt = luci.util.exec("/sbin/ifconfig 2>/dev/null")
	local ifc = {}
...
function user.setpasswd(username, password)
	...
	if ret == 0 and username == "root" and #password > 7 and password:match("([0-9])") and password:match("([a-zA-Z])") then
		os.execute("logger passwd is good")
		os.execute("uci set system.fon.pass_good=1")
	else
		os.execute("logger passwd is bad")
		os.execute("uci delete system.fon.pass_good")
	end
