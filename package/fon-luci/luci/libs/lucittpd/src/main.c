...
static void run_luci(int sockfd, char *remote_addr, char *server_addr)
{
	lua_pushstring(ctx.L, remote_addr);
	lua_setfield(ctx.L, LUA_GLOBALSINDEX, "webuci_REMOTE_ADDR");

	lua_pushstring(ctx.L, server_addr);
	lua_setfield(ctx.L, LUA_GLOBALSINDEX, "webuci_SERVER_ADDR");
...
int main(int argc, char **argv)
{
	...
	socklen_t sin_size;
	int yes = 1;
	struct sockaddr_in myaddr, local_addr, their_addr;
	char local_ip[INET6_ADDRSTRLEN+1], remote_ip[INET6_ADDRSTRLEN+1];
	...
	while(1)
	{
		...
		inet_ntop(their_addr.sin_family, (void*)&their_addr.sin_addr,
				remote_ip, sizeof remote_ip);

		/* local address */
		memset(local_ip, 0, sizeof local_ip);
		sin_size = sizeof local_addr;
		getsockname(new_fd, (struct sockaddr*)&local_addr, &sin_size);
		inet_ntop(local_addr.sin_family, (void*)&local_addr.sin_addr,
		...
		if(!fork())
		{
			...
			run_luci(new_fd, remote_ip, local_ip);
			cleanup_luci();
			close(new_fd);
