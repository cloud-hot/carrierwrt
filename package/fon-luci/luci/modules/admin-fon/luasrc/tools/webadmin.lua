...
function date_format(secs)
	...
	if days > 0 then
		return string.format("%.0fd %02.0fh %02.0fm %02.0fs", days, hour, mins, secs)
	else
		if hour > 0 then
			return string.format("%02.0fh %02.0fm %02.0fs", hour, mins, secs)
		else
			return string.format("%02.0fm %02.0fs", mins, secs)
		end
	end
end

function date_format_short(secs)
	local suff = {"min", "h", "d"}
	local mins = 0
	local hour = 0
	local days = 0
	...
	if days > 0 then
		return string.format("%.0fd %02.0fh", days, hour)
	else
		if hour > 0 then
			return string.format("%02.0fh %02.0fm", hour, mins)
		else
			return string.format("%02.0fm %02.0fs", mins, secs)
		end
	end
...
function foreach_uci(config, type, head, item, tail, priv)
	local uci = luci.model.uci.cursor_state()
	local templates = {}

	uci:load(config);
	uci:foreach(config, type, function(section)
		templates[#templates + 1] = { template = item, params={section=section, priv=priv, id=#templates}}
	end)

	priv.count = #templates
	luci.http.prepare_content("text/html")
	luci.template.render(head, {priv=priv})

	for i = 1, #templates, 1 do
		luci.template.render(templates[i].template, templates[i].params)
	end

	luci.template.render(tail,{priv=priv})
end
