...
function index()
	...
	local page  = node("fon_devices", "fon_plugins")
	...
	local page  = node("fon_devices", "fon_plugins", "upgrade")
...
function dashboard()
	...
	local plugins = uci:get_all("plugfons")
	local objects = {}

	for k, v in util.spairs(plugins, function (a,b)
		return (tonumber(plugins[a].order) or 100) < (tonumber(plugins[b].order) or 100)
	end) do
		if v.order then
			local url = v.href
			local node = dsp.node(url)
			--local url = luci.util.split((v.href or ""), "/")
			if node == nil then
				local url = luci.util.split((v.href or ""), "/")
			else
			end
			...
			then
				obj.csscl = v.name
				if node.icon_select then
					obj.iconl = cfg.main.mediaurlbase.."/"..node.icon_select()
				else
					obj.iconl = cfg.main.resourcebase.."/"..v.dashicon
				end
			else
				obj.csscl = "%s" % v.id
			end
			obj.aclass = v[".name"]
			obj.target = dsp.build_url(v.href)
			obj.title = node and node.title or v.name
			objects[#objects+1] = obj
		end
	end

	tpl.render("fon/master_dashboard", {objects=objects})
			...
			if ple.plugins[i].name == (update or install) and ple.plugins[i].device == device and
				...
				if ple.plugins[i].firmware then
					file = feed..ple.plugins[i].name.."_"..ple.plugins[i].version.."_"..ple.plugins[i].device.."_"..ple.plugins[i].firmware.."_"..ple.plugins[i].signature..".tar.gz"
                                ...
                                allow_unsigned = false -- Never allow unsigned tarballs to be installed from the download server
