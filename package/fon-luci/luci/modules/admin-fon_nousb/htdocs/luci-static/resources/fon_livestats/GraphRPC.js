function Graph(container, id, options, transform, legend) {
	...
	// Color Scheme
	this.options.colorScheme = this.options.colorScheme ||
	PlotKit.Base.palette(PlotKit.Base.baseColors()[5]);
...
Graph.prototype.draw = function( options ) {
	if( this.layout ) {
		...
		this.options.legendStyle = this.options.legendStyle || 'li';

		legend = new LegendRenderer(this.legend, this.layout, this.options);
		legend.render();
	}
}
