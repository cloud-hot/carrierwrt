...
f = SimpleForm("password",
	translate("yt_passwd_title", "Youtube username and password"),
	translate("yt_passwd_desc", "Please fill in with your youtube username and password to let La Fonera upload your videos."))
f.cancel = false
...
function pw.validate(self, value, section)
	...
	local auth = gdata.get_auth_token(user:formvalue(section), value, "youtube")
	if not(auth) then
		set_error()
	end
	return value
...
function f.handle(self, state, data)
	if state == FORM_VALID  and data.user and data.pw and #data.user > 0 and #data.pw > 0 then
		local gdata = require "luci.fon.gdata"
		gdata.adduser("youtube", data.user, data.pw)
	end
	return true
end
